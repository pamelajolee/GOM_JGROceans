function [center_data] = track_LCFE_center(dates, adt_mask)
...
for d = 1:length(dates) 
    ...
    try
        % Retrieving data
        [lon_adt, lat_adt, ~, adt_masked] = gom_adt(years, months, days);
    catch ME1 %#ok<NASGU>
    ...
    adt_masked(~adt_mask) = NaN;  

    % Convert lon/lat grids to vectors for contourc
    xvec = double(lon_adt(1,:));   % 1D
    yvec = double(lat_adt(:,1));   % 1D 
    
    % Extract contour (using contourc so it doesn't plot and screw up final figure)
    C_LCFE = contourc(xvec, yvec, adt_masked, [ssh_LCFE ssh_LCFE]);
    C_LC   = contourc(xvec, yvec, adt_masked, [ssh_LC ssh_LC]);
