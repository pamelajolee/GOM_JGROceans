...
%%
function [lon_hycom, lat_hycom, w_vel, depth] = gom_hycom_w_vel(year, month, day)
...
if exist("fileN1", "var")
    ...
    w_total = ncread(fname1,'w_velocity');
    w_vel = permute(w_total, [2 1 3]); % reorder the matrix so that its [lat lon depth]

    depth = ncread(fname1, 'Depth');

    % % Apply the mask to remove shollower than 200 m, the Caribbean waters, and Atlantic
    % w_masked = w_vel_total;
    % w_masked(id_ssh_nan)=nan;
    % 
    % % Calculate and remove seasonal signal from the ssh masked region
    % %% Do I want to remove seasonal signal from HYCOM?
    % w_mean_gom = nanmean(w_masked(:));
    % % clear ssh_masked
    % 
    % w_vel = w_vel_total - w_mean_gom; % full region with mean removed from >-200
    % w_masked = w_masked - w_mean_gom; % only GoM to plot LCFE and not Gulf Stream
