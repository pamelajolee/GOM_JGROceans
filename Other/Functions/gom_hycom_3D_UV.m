%% FUNCTION - Retrieves u and v sea water velocities 
...
function [lon_hycom, lat_hycom, u_vel, v_vel, depth] = gom_hycom_3D_UV(year, month, day)
...
if exist("fileN1", "var")
    ...
    u_total = ncread(fname1,'u');
    u_vel = permute(u_total, [2 1 3]); % reorder the matrix so that its [lat lon depth]

    v_total = ncread(fname1, 'v');
    v_vel = permute(v_total, [2 1 3]); 
